...
function InfoCard({img, location, title, description,
    ...
    return (
        ...
        hover:opacity-80 hover:shadow-lg transition duration-200 ease-out first:border-t'>
            <div className='relative h-24 w-40 md:h-40 md:w-80
            flex-shrink-0'>
                <Image className='rounded-2xl' src={img} layout="fill" objectFit='cover' priority="true" />
            </div>

            <div className='flex flex-col flex-grow pl-5'>
                <div className='flex justify-between'>
                    <p>{location}</p>
                    <HeartIcon className='h-7 cursor-pointer active:hidden' /> 
                </div>
